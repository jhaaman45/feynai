...
def helper(dis):
    ...
    pre = list(filter(pd.notna, pre))  # Remove NaN values
...
# Streamlit app
st.set_page_config(page_title="SymptomSolver", page_icon=":hospital:", layout="wide")
st.sidebar.image("SymptomSolver.png", use_column_width=True)
st.sidebar.title("SymptomSolver")
st.sidebar.markdown("### Your Personal Health Detective")
st.sidebar.markdown("---")
st.sidebar.markdown("#### Developer")
st.sidebar.markdown("Anirban Bhattacharjee")
st.sidebar.markdown("[LinkedIn Profile](https://www.linkedin.com/in/anirban-bhattacharjee-10063a150/)")
...
st.title("SymptomSolver")
st.subheader("Your Personal Health Detective")
st.write("Enter your symptoms separated by commas (e.g., headache, nausea, fatigue):")
...
if st.button("Predict"):
    if symptoms_input:
        ...
        if not all(symptom in symptoms_dict for symptom in user_symptoms):
            st.error("Some symptoms are misspelled or not recognized.")
        else:
            ...
            with st.expander("Recommended Diet"):
                st.write(f"**Recommended Diet:** {', '.join(rec_diet)}")
                
            with st.expander("Workout"):
                st.write(f"**Workout:** {', '.join(workout)}")
    else:
